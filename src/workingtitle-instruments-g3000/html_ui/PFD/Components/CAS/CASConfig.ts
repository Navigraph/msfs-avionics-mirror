...
export class CASConfig implements Config {
  ...
  /**
   * The maximum number of unscrollable CAS messages to display when the PFD is in full mode. Only applies if the
   * display version is 2.
   */
  public readonly unscrollableAlertCountFullScreen: number;

  /**
   * The maximum number of unscrollable CAS messages to display when the PFD is in split mode. Only applies if the
   * display version is 2.
   */
  ...
  constructor(element: Element | undefined, pfdLayoutConfig: PfdLayoutConfig) {
    ...
    this.version = data.version ?? inheritData?.version ?? 1;
    ...
    if (this.version === 2) {
      ...
      this.unscrollableAlertCountFullScreen = 0;
      this.unscrollableAlertCountSplitScreen = 0;
    }
  }
...
class CASConfigData {
  ...
  constructor(element: Element | undefined) {
    ...
    switch (element.getAttribute('version')) {
      case '1':
        this.version = 1;
        break;
      case '2':
        ...
        break;
      default:
        console.warn('Invalid CASConfig definition: unrecognized "version" option (must be 1 or 2)');
    }
