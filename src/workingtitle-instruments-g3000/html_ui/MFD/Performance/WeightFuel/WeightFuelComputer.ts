import {
  ...
  MappedSubject, MappedSubscribable, SimVarValueType, Subject, Subscribable, SubscribableUtils, Subscription, UnitType
} from '@microsoft/msfs-sdk';

import { Fms, FmsPositionMode, FmsPositionSystemEvents, LNavDataEvents } from '@microsoft/msfs-garminsdk';

import {
  FuelTotalizerControlEvents, FuelTotalizerEvents, G3000FlightPlannerId, WeightFuelSimVars, WeightFuelUserSettings
...
export class WeightFuelComputer {
  ...
  private readonly basicOperatingWeight = this.calculateLoadWeights
    ...
    : this.weightFuelSettingManager.getSetting('weightFuelBasicOperating');
  ...
  private readonly totalPassengerWeight = this.calculateLoadWeights
    ...
    : this.weightFuelSettingManager.getSetting('weightFuelTotalPassenger');
  ...
  private readonly zeroFuelWeight = this.calculateLoadWeights
    ...
    : this.weightFuelSettingManager.getSetting('weightFuelZeroFuel');

  private readonly rampWeight = MappedSubject.create(
    ([zeroFuel, initialFob]) => initialFob < 0 ? -1 : zeroFuel + initialFob,
    this.zeroFuelWeight,
    this.initialFobWeightSetting
  ).pause();

  private readonly takeoffWeight = MappedSubject.create(
    ([zeroFuelWeight, rampWeight, taxiFuel]) => rampWeight < 0 ? -1 : Math.max(rampWeight - taxiFuel, zeroFuelWeight),
  ...
  private readonly calculatedWeights: MappedSubscribable<any>[] = this.calculateLoadWeights
    ? [
      this.basicOperatingWeight as MappedSubscribable<number>,
      this.totalPassengerWeight as MappedSubscribable<number>,
      this.zeroFuelWeight as MappedSubscribable<number>,
      this.rampWeight,
      this.takeoffWeight,
      this.fuelOnBoardWeight,
      this.aircraftWeight
  ...
  public init(paused = false): void {
    ...
    this.settingPipes.push(
      this.rampWeight.pipe(this.weightFuelSettingManager.getSetting('weightFuelRamp')),
      this.takeoffWeight.pipe(this.weightFuelSettingManager.getSetting('weightFuelTakeoff')),
    );
    ...
    this.publishSubs.push(
      ...
      this.excessFuel.sub(weight => { SimVar.SetSimVarValue(WeightFuelSimVars.ExcessFuel, SimVarValueType.Pounds, isNaN(weight) ? Number.MIN_SAFE_INTEGER - 1 : weight); }, true)
    );
  ...
  public resume(): void {
    ...
    for (const pipe of this.settingPipes) {
      pipe.resume(true);
    }
    for (const sub of this.publishSubs) {
      sub.resume(true);
    }
