import {
  APRadioNavInstrument, ArrayUtils, AuralAlertSystem, AuralAlertSystemWarningAdapter, AuralAlertSystemXmlAdapter, CasSystem, CasSystemLegacyAdapter,
  ChecklistManager, ClockEvents, CompositeLogicXMLHost, ConsumerValue, ControlEvents, DefaultXmlAuralAlertParser, FlightPlanCalculatedEvent,
  FlightTimerInstrument, FlightTimerMode, FSComponent, GameStateProvider, GpsSynchronizer, GPSSystemState, LNavComputer, LNavObsManager, MinimumsManager,
  NavComInstrument, NavSourceType, PluginSystem, SetSubject, SimVarValueType, SoundServer, Subject, TrafficInstrument, UserSetting, Value, Vec2Math,
  VNavControlEvents, VNode, Wait, XMLWarningFactory, XPDRInstrument
...
import {
  ...
  Fms, FmsPositionMode, FmsPositionSystemSelector, GarminAPConfig, GarminAPStateManager, GarminAPUtils, GarminGoAroundManager,
  GarminHeadingSyncManager, GarminNavToNavComputer, GarminObsLNavModule, GarminTimerControlEvents, GarminTimerManager, GarminXpdrTcasManager,
  MapTerrainWxSettingCompatManager, MinimumsUnitsManager, NavdataComputer, TrafficOperatingModeManager, TrafficOperatingModeSetting, TrafficSystemType,
...
import {
  ...
  ConnextWeatherPaneView, DisplayPaneContainer, DisplayPaneIndex, DisplayPaneViewFactory, DisplayPaneViewKeys,
  EspUserSettings, FlightPlanListManager, FlightPlanStore, FuelTotalizer, G3000Autopilot, G3000ComRadioUserSettings,
  G3000EspDefinition, G3000FilePaths, G3000UserSettingSaveManager, GpsStatusDataProvider, GpsStatusPane,
  InitializationProcess, InstrumentBackplaneNames, MapUserSettings, MfdNavDataBarUserSettings, NavigationMapPaneView,
...
export class WTG3000MfdInstrument extends WTG3000FsInstrument {
  ...
  private readonly apConfig = new GarminAPConfig(this.bus, this.fms, this.verticalPathCalculator, {
    deactivateAutopilotOnGa: this.config.autopilot.deactivateAutopilotOnGa,
    ...
    toPitchAngle: this.config.autopilot.toOptions.targetPitchAngle,
    gaPitchAngle: this.config.autopilot.gaOptions.targetPitchAngle,
  });
  ...
  private readonly weightFuelComputer = new WeightFuelComputer(this.bus, this.fms, this.iauIndex, !this.config.performance.isWeightBalanceSupported);
  ...
  private createEsp(): void {
    let def: G3000EspDefinition | undefined = undefined;
  ...
  private async initNavigationLoop(): Promise<void> {
    ...
    sub.on('simTimeHiFreq').handle(() => {
      ...
      if (isPaused) {
        ...
        if (this.esp) {
          this.espDataProvider!.update(realTime);
          this.esp.update();
        }
      }
    });
