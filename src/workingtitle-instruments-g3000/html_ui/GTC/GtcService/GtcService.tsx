import {
  ArrayUtils, ComponentProps, ConsumerSubject, DebounceTimer, DisplayComponent, EventBus, FSComponent, HEvent,
  NodeReference, ObjectSubject, SetSubject, Subject, Subscribable, Subscription, UserSetting, VNode
...
export class GtcService {
  ...
  /**
   * Opens a view as a vertical GTC NAV/COM popup. The opened view will be brought to the top of the overlay view stack
   * of the currently selected control mode as the active view.
   * @param key The key of the view to open.
   * @param popupType The type of popup to open the view as. Defaults to `'normal'`.
   * @param backgroundOcclusion The occlusion type applied to views beneath the popup. If `'none'` is chosen, then the
   * popup will not prevent mouse events from reaching views beneath it (unless the mouse event was triggered on an
   ...
   * @deprecated Please use `openOverlayPopup()` instead.
   */
  public openVerticalNavComPopup<T extends GtcView = GtcView>(key: string, popupType: GtcPopupType = 'normal', backgroundOcclusion: GtcViewOcclusionType = 'darken'): GtcViewEntry<T> {
    ...
    return this.openOverlayPopup(key, popupType, backgroundOcclusion);
  }
  ...
  /**
   * Clears the current vertical GTC NAV/COM view stack (i.e. the overlay view stack of the currently selected control
   * mode), closing all open popups in the stack and resetting its history.
   * @returns The active view entry after the operation is complete.
   * @throws Error if this service's parent GTC has the horizontal orientation.
   * @deprecated Please use `clearOverlayViewStack()` instead.
   */
  public clearVerticalNavComViewStack(): GtcViewEntry {
    ...
    return this.clearOverlayViewStack();
  }
