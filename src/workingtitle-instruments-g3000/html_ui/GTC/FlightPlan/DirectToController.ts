import {
  BasicNavAngleUnit, Facility, FacilityUtils, FacilityWaypoint, FacilityWaypointUtils, FlightPlan, FlightPlanUtils,
  ICAO, IcaoValue, LegDefinition, LegTurnDirection, MagVar, Subject, UnitType, Waypoint
...
import { HoldController } from '../Pages/HoldPage/HoldController';
import { HoldCourseDirection, HoldLegMode } from '../Pages/HoldPage/HoldStore';
...
export class DirectToController {
  ...
  private initializeIcao = ICAO.emptyValue();
  ...
  public async initializeTarget(dtoData: DirectToInputData): Promise<void> {
    ...
    let targetIcao = ICAO.emptyValue();

    let leg: LegDefinition | undefined | null;
    let nextLeg: LegDefinition | undefined | null;

    if (primaryPlan && dtoData.segmentIndex !== undefined && dtoData.segmentLegIndex !== undefined) {
      ...
      const icao = dtoData.facility?.icaoStruct ?? ICAO.emptyValue();
      targetIcao = icao;
      ...
      if (dtoState === DirectToState.TOEXISTING && primaryPlan) {
        ...
        if (ICAO.isValueEmpty(targetIcao) || ICAO.valueEquals(dtoLeg.leg.fixIcaoStruct, targetIcao)) {
          leg = dtoLeg;
          this.store.directToExistingData.set({
            icao: leg.leg.fixIcaoStruct,
        ...
        if (ICAO.isValueEmpty(targetIcao) || ICAO.valueEquals(dtoLeg.leg.fixIcaoStruct, targetIcao)) {
          leg = dtoLeg;
          targetIcao = leg.leg.fixIcaoStruct;
    ...
    if (
      leg
      && nextLeg
      && FlightPlanUtils.isHoldLeg(nextLeg.leg.type)
      && ICAO.valueEquals(leg.leg.fixIcaoStruct, nextLeg.leg.fixIcaoStruct)
    ) {
      const facility = await this.fms.facLoader.tryGetFacility(ICAO.getFacilityTypeFromValue(nextLeg.leg.fixIcaoStruct), nextLeg.leg.fixIcaoStruct);
    ...
    if (!ICAO.isValueEmpty(targetIcao)) {
      if (dtoData.facility && ICAO.valueEquals(targetIcao, dtoData.facility.icaoStruct)) {
        waypoint = this.waypointCache.get(dtoData.facility);
      } else {
        const fac = await this.fms.facLoader.tryGetFacility(ICAO.getFacilityTypeFromValue(targetIcao), targetIcao);
   ...
   * @param targetIcao The ICAO of the desired Direct To target fix. If undefined or the empty ICAO is specified, then
   * the chosen target fix does not have to match any particular ICAO.
   * @returns A Direct To Existing data object describing the chosen target, or `null` if a valid target could not be
   * found.
   */
  private searchForValidDtoExistingLeg(segmentIndex: number, segmentLegIndex: number, targetIcao?: IcaoValue): DirectToExistingData | null;
  /**
   ...
   * found.
   */
  private searchForValidDtoExistingLeg(legIndex: number, targetIcao?: IcaoValue): DirectToExistingData | null;
  // eslint-disable-next-line jsdoc/require-jsdoc
  private searchForValidDtoExistingLeg(arg1: number, arg2?: number | IcaoValue, arg3?: IcaoValue): DirectToExistingData | null {
    const plan = this.fms.getPrimaryFlightPlan();
    const legIndex = typeof arg2 === 'number'
      ? plan.getSegment(arg1).offset + arg2
      : arg1;

    const targetIcao = (typeof arg2 === 'number' ? arg3 : arg2) ?? ICAO.emptyValue();
    ...
    for (let i = legIndex; i < len; i++) {
      ...
      if (
        (ICAO.isValueEmpty(targetIcao) || ICAO.valueEquals(targetIcao, currSegment.legs[currSegmentLegIndex].leg.fixIcaoStruct))
        && this.fms.canDirectTo(currSegmentIndex, currSegmentLegIndex)
      ) {
  ...
  private onWaypointChanged(waypoint: Waypoint | null): void {
    ...
    if ((this.initializeIcao === null && facility === null) || ICAO.valueEquals(this.initializeIcao, facility?.icaoStruct ?? ICAO.emptyValue())) {
      return;
    }

    if (!facility) {
      this.initializeIcao = ICAO.emptyValue();
      this.store.userCourseMagnetic.set(undefined);
      this.store.holdInfo.set(undefined);
      this.store.directToExistingData.set(null);
    ...
    } else if (!ICAO.valueEquals(facility.icaoStruct, this.store.directToExistingData.get()?.icao ?? ICAO.emptyValue())) {
    ...
    if (facility) {
      ...
      if (directToExistingData) {
        ...
        if (holdInfo) {
          ...
          if (nextLeg && FlightPlanUtils.isHoldLeg(nextLeg.leg.type) && ICAO.valueEquals(nextLeg.leg.fixIcaoStruct, leg.leg.fixIcaoStruct)) {
   ...
   */
  private static createDtoExistingData(plan: FlightPlan, segmentIndex: number, segmentLegIndex: number): DirectToExistingData {
    return {
      icao: plan.getSegment(segmentIndex).legs[segmentLegIndex].leg.fixIcaoStruct,
      segmentIndex,
      segmentLegIndex,
    };
  }
