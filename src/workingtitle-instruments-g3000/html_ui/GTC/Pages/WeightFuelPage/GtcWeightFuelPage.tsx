import {
  AdcEvents, AirportFacility, ConsumerSubject, ConsumerValue, EngineEvents, FSComponent, GNSSEvents, LNavDataEvents,
...
export class GtcWeightFuelPage extends GtcView<GtcWeightFuelPageProps> {
  ...
  private readonly excessFuelSource = ConsumerSubject.create(this.evtSub.on('weightfuel_excess_fuel').atFrequency(1), -1).pause();
  ...
  public override onAfterRender(): void {
    ...
    const negativeToNaN = (value: number): number => value < 0 ? NaN : value;
    const minSafeIntegerToNaN = (value: number): number => value <= Number.MIN_SAFE_INTEGER ? NaN : value;

    this.fuelOnBoardWeightSource.pipe(this.fuelOnBoardWeightNUS, negativeToNaN);
    this.aircraftWeightSource.pipe(this.aircraftWeightNUS, negativeToNaN);
    this.holdingFuelSource.pipe(this.estHoldingFuelWeightNUS, negativeToNaN);
    this.landingFuelSource.pipe(this.estLandingFuelWeightNUS, minSafeIntegerToNaN);
    this.landingWeightSource.pipe(this.estLandingWeightNUS, negativeToNaN);
    this.excessFuelSource.pipe(this.excessFuelWeightNUS, minSafeIntegerToNaN);
    ...
    this.zeroFuelWeightSetting.sub(zfw => {
      const maxZfw = this.props.weightLimits.maxZeroFuel.asUnit(UnitType.POUND);
      this.zeroFuelWeightClasses.toggle(CAUTION_CLASS, zfw >= 0 && zfw > maxZfw);
    }, true);

    this.aircraftWeightSource.sub((tow: number): void => {
      const mtow = this.props.weightLimits.maxTakeoff.asUnit(UnitType.POUND);
      const mtowExceeded = tow >= 0 && tow > mtow;
      this.aircraftWeightClasses.toggle(CAUTION_CLASS, mtowExceeded);
      this.overweightCautionClasses.toggle('hidden', !mtowExceeded);
    }, true);

    this.landingWeightSource.sub((elw: number): void => {
      const mlw = this.props.weightLimits.maxLanding.asUnit(UnitType.POUND);
      this.estLandingWeightClasses.toggle(CAUTION_CLASS, elw >= 0 && elw > mlw);
    }, true);
    ...
    MappedSubject.create(
      ...
      const isValid = landingFuel > Number.MIN_SAFE_INTEGER && excessFuel > Number.MIN_SAFE_INTEGER;

      const isWarning = isValid && landingFuel <= 0;
      ...
      const isCaution = isValid && !isWarning && excessFuel <= 0;
  ...
  private async onFuelOnBoardPressed(): Promise<void> {
    ...
    const result = await this.openWeightDialog({
      title: 'Fuel On Board',
      initialValue: currentFobWeight < 0 ? 0 : currentFobWeight,
      initialUnit: UnitType.POUND,
