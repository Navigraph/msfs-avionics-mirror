import {
  ArraySubject, ComponentProps, ConsumerSubject, DisplayComponent, FSComponent, NodeReference, NumberFormatter,
  NumberUnitSubject, SimVarValueType, Subject, Subscribable, Subscription, UnitType, UserSetting, UserSettingManager,
  VNode
...
class GtcWeightBalancePageEmptyWeightTab extends DisplayComponent<GtcWeightBalancePageTabContentProps> implements GtcWeightBalancePageTabContent {
  ...
  private readonly basicEmptyArmSetting = this.props.weightBalanceSettingManager.getSetting('weightBalanceBasicEmptyArm');
  ...
  private readonly basicEmptyMacValueText = this.props.weightBalanceConfig.macArm
    ? this.basicEmptyArmSetting.map(arm => {
  ...
  private readonly subscriptions: Subscription[] = this.basicEmptyMacValueText
    ? [this.basicEmptyArmUnit, this.basicEmptyMacValueText]
    : [this.basicEmptyArmUnit];
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    this.subscriptions.push(
      ...
      this.basicEmptyArmSetting.pipe(this.basicEmptyArmValue, true)
    );
  }
...
class GtcWeightBalancePageFuelTab extends DisplayComponent<GtcWeightBalancePageTabContentProps> implements GtcWeightBalancePageTabContent {
  ...
  private readonly fuelOnBoardWeightSource = ConsumerSubject.create(null, -1).pause();
  ...
  private readonly holdingFuelSource = ConsumerSubject.create(null, -1).pause();
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    const sub = this.props.gtcService.bus.getSubscriber<WeightFuelEvents>();
    this.fuelOnBoardWeightSource.setConsumer(sub.on('weightfuel_fob_weight').atFrequency(1));
    this.holdingFuelSource.setConsumer(sub.on('weightfuel_holding_fuel').atFrequency(1));

    const negativeToNaN = (value: number): number => value < 0 ? NaN : value;

    this.fuelOnBoardWeightSource.pipe(this.fuelOnBoardWeightValue, negativeToNaN);
    this.holdingFuelSource.pipe(this.estHoldingFuelWeightValue, negativeToNaN);
