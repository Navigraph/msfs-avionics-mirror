...
import { FacilityType, FSComponent, ICAO, NodeReference, RunwayUtils, Subscribable, UnitType, VNode } from '@microsoft/msfs-sdk';
  ...
  private async autoSelectDestination(): Promise<void> {
    ...
    if (destinationFacility !== undefined) {
      if (destinationRunway !== undefined) {
        this.originDestIcaoSetting.value = RunwayUtils.getRunwayFacilityIcao(destinationFacility, destinationRunway);
      } else {
        this.originDestIcaoSetting.value = destinationFacility.icao;
      }
    } else {
