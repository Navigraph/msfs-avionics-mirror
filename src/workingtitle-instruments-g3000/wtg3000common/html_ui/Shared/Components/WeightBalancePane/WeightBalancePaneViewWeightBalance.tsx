...
export class WeightBalancePaneViewWeightBalance extends DisplayComponent<WeightBalancePaneViewWeightBalanceProps> implements WeightBalancePaneViewPanel {
  ...
  private readonly fobSource = ConsumerSubject.create(null, -1).pause();
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    this.fobSource.setConsumer(sub.on('weightfuel_fob_weight').withPrecision(0));
    ...
    this.fobSource.pipe(this.fobValue, negativeToNaN);
    this.currentWeightSource.pipe(this.currentWeightValue, negativeToNaN);
    this.currentArmSource.pipe(this.currentArmValue, value => value <= Number.MIN_SAFE_INTEGER ? NaN : value);

    this.subscriptions.push(
      this.props.weightFuelSettingManager.getSetting('weightFuelBasicEmpty').pipe(this.basicEmptyWeightValue, true),
      this.props.weightFuelSettingManager.getSetting('weightFuelZeroFuel').pipe(this.zeroFuelWeightValue, negativeToNaN, true),
      this.props.weightFuelSettingManager.getSetting('weightFuelRamp').pipe(this.rampWeightValue, negativeToNaN, true),
      this.props.weightFuelSettingManager.getSetting('weightFuelTaxi').pipe(this.taxiFuelValue, true),
      this.props.weightFuelSettingManager.getSetting('weightFuelTakeoff').pipe(this.takeoffWeightValue, negativeToNaN, true)
    );
  }
