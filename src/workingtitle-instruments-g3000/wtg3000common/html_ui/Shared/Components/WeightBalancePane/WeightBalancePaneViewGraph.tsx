import {
  ArrayUtils, ClippedPathStream, ComponentProps, ConsumerSubject, CssTransformBuilder, CssTransformChain,
  CssTransformSubject, CssTranslate3dTransform, CssTranslateTransform, DisplayComponent, EventBus, FSComponent,
  MappedSubject, MathUtils, MutableSubscribable, NullPathStream, PathStream, Subject, SubscribableMapFunctions,
  Subscription, UnitType, UserSettingManager, VNode, VecNMath, VecNSubject
...
export class WeightBalancePaneViewGraph extends DisplayComponent<WeightBalancePaneViewGraphProps> implements WeightBalancePaneViewPanel {
  ...
  private readonly zeroFuelWeight = this.props.weightFuelSettingManager.getSetting('weightFuelZeroFuel');
  private readonly rampWeight = this.props.weightFuelSettingManager.getSetting('weightFuelRamp');
  private readonly zeroFuelMoment = this.props.weightBalanceSettingManager.getSetting('weightBalanceZeroFuelMoment');

  private readonly takeoffWeightSetting = this.props.weightFuelSettingManager.getSetting('weightFuelTakeoff');
  private readonly takeoffArmSetting = this.props.weightBalanceSettingManager.getSetting('weightBalanceTakeoffArm');
  private readonly takeoffMarkerMode = MappedSubject.create(
    ([envelopeDef, weight, arm]): MarkerMode => {
      if (weight < 0 || arm <= Number.MIN_SAFE_INTEGER) {
        return 'none';
      }
  ...
  private readonly currentWeightSource = ConsumerSubject.create(null, -1).pause();
  private readonly currentArmSource = ConsumerSubject.create(null, Number.MIN_SAFE_INTEGER - 1).pause();
  ...
  private readonly currentMarkerMode = MappedSubject.create(
    ([envelopeDef, weight, arm, isCurrentMarkerOffscale]): MarkerMode => {
      if (isCurrentMarkerOffscale || weight < 0 || arm <= Number.MIN_SAFE_INTEGER) {
        return 'none';
      }
  ...
  private readonly landingWeightSource = ConsumerSubject.create(null, -1).pause();
  private readonly landingArmSource = ConsumerSubject.create(null, Number.MIN_SAFE_INTEGER - 1).pause();
  ...
  public onAfterRender(): void {
    ...
    this.canvasRefreshSubs.push(
      ...
      this.zeroFuelMoment.sub(scheduleRefreshPlot, false, true),
      this.rampWeight.sub(scheduleRefreshPlot, false, true)
    );
  }
  ...
  private refreshCanvas(): void {
    ...
    const zeroFuelWeight = this.zeroFuelWeight.value;
    const rampWeight = this.rampWeight.value;
    if (zeroFuelWeight >= 0 && rampWeight > zeroFuelWeight) {
      ...
      this.clipPathStream.beginPath();
      this.drawCgPath(this.clipPathStream, this.zeroFuelMoment.value, zeroFuelWeight, rampWeight, this.scaleDef, x0, y0, x1, y1);
  ...
  private updateMarkers(): void {
    ...
    this.updateMarkerPosition(
      this.takeoffWeightSetting.value,
      this.takeoffArmSetting.value,
      this.scaleDef,
      x0, y0,
      x1, y1,
   ...
   * @param weight The marker's weight, in pounds.
   * @param arm The marker's moment arm, in inches.
