import {
  ComponentProps, ConsumerSubject, DisplayComponent, EventBus, FSComponent, MappedSubject, NumberFormatter, NumberUnitSubject,
...
export class WeightBalancePaneViewEstimatedWeight extends DisplayComponent<WeightBalancePaneViewEstimatedWeightProps> implements WeightBalancePaneViewPanel {
  ...
  private readonly landingFuelSource = ConsumerSubject.create(null, Number.MIN_SAFE_INTEGER).pause();
  ...
  private readonly holdFuelSource = ConsumerSubject.create(null, -1).pause();
  private readonly holdFuelValue = NumberUnitSubject.create(UnitType.POUND.createNumber(NaN));

  private readonly excessFuelSource = ConsumerSubject.create(null, Number.MIN_SAFE_INTEGER).pause();
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    this.landingFuelSource.setConsumer(sub.on('weightfuel_landing_fuel').withPrecision(0));
    this.holdFuelSource.setConsumer(sub.on('weightfuel_holding_fuel').withPrecision(0));
    this.excessFuelSource.setConsumer(sub.on('weightfuel_excess_fuel').withPrecision(0));
    ...
    this.landingWeightSource.pipe(this.landingWeightValue, negativeToNaN);
    this.landingFuelSource.pipe(this.landingFuelValue, minSafeIntegerToNaN);
    this.holdFuelSource.pipe(this.holdFuelValue, negativeToNaN);
    this.excessFuelSource.pipe(this.excessFuelValue, minSafeIntegerToNaN);
