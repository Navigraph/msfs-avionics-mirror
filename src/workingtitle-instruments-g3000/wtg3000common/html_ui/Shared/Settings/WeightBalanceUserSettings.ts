...
export type WeightBalanceUserSettingTypes = {
  ...
  weightBalanceBasicEmptyArm: number;

  /** The calculated zero fuel CG moment, in pound-inches. */
  weightBalanceZeroFuelMoment: number;

  /**
   * The calculated takeoff CG moment arm, in inches. A value less than or equal to {@link Number.MIN_SAFE_INTEGER}
   * indicates the quantity could not be calculated.
   */
  weightBalanceTakeoffArm: number;
};
...
export class WeightBalanceUserSettingManager implements UserSettingManager<WeightBalanceUserSettingTypes> {
  ...
  public constructor(bus: EventBus, config: WeightBalanceConfig) {
    ...
    settingDefs.push(
      ...
      {
        name: 'weightBalanceBasicEmptyArm',
        defaultValue: 0
      },
      {
        name: 'weightBalanceZeroFuelMoment',
        defaultValue: 0
      },
      {
        name: 'weightBalanceTakeoffArm',
