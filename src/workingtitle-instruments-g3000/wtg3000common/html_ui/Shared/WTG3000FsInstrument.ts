...
import {
  AdcPublisher, AhrsPublisher, ArrayUtils, AutopilotInstrument, AvionicsSystem, BaseInstrumentPublisher, Clock, ConsumerSubject, ControlSurfacesPublisher,
  DebounceTimer, DefaultChecklistStateProvider, EISPublisher, EventBus, FacilityLoader, FacilityRepository, FlightPathAirplaneSpeedMode, FlightPathCalculator,
  FlightPlanner, FlightTimerPublisher, FsInstrument, GameStateProvider, GNSSPublisher, GPSSatComputer, HEventPublisher, InstrumentBackplane,
  LNavObsSimVarPublisher, LNavSimVarPublisher, MinimumsSimVarPublisher, NavComSimVarPublisher, PluginSystem, SBASGroupName, SimVarValueType,
  SmoothingPathCalculator, Subject, VNavSimVarPublisher, Wait
} from '@microsoft/msfs-sdk';

import {
  AdcSystem, AglSystem, AhrsSystem, AoaSystem, DefaultMinimumsDataProvider, DefaultTerrainSystemStateDataProvider, Fms,
  FmsPositionSystem, FmsPositionSystemSelector, GarminAPSimVarPublisher, GarminChecklistItemTypeDefMap,
  GarminNavSimVarPublisher, GarminSpeedConstraintStore, GarminVNavUtils, GpsNavSource, GpsReceiverSelector,
...
import { G3000WeightBalancePublisher } from './Performance/WeightBalance/G3000WeightBalanceEvents';
...
export abstract class WTG3000FsInstrument implements FsInstrument {
  ...
  protected readonly weightBalancePublisher = this.config.performance.isWeightBalanceSupported
    ? new G3000WeightBalancePublisher(this.bus)
    : undefined;
  ...
  constructor(
    ...
    if (this.weightBalancePublisher) {
      this.backplane.addPublisher(InstrumentBackplaneNames.WeightBalance, this.weightBalancePublisher);
    }
  ...
  protected createSystems(): void {
    ...
    const aglFmsPosSelector = new FmsPositionSystemSelector(this.bus, ArrayUtils.range(this.config.iauDefs.count, 1), 1);
    aglFmsPosSelector.init();
    this.systems.push(new AglSystem(1, this.bus, globalSystemsPower, { fmsPosIndex: aglFmsPosSelector.selectedIndex }));
    if (this.config.sensors.radarAltimeterDefinition !== undefined) {
