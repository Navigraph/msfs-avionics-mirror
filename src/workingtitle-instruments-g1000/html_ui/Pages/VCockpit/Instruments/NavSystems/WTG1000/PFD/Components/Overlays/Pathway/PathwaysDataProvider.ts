import {
  AdcEvents, AhrsEvents, APEvents, ApproachGuidanceMode, APVerticalModes, BaseControlSurfacesEvents, CircleVector, ClockEvents, ConsumerSubject, EventBus,
  FixTypeFlags, FlightPathUtils, FlightPlanner, GeoCircle, GeoPoint, GNSSEvents, LegDefinition, LegType, LerpLookupTable, LNavEvents, LNavTransitionMode,
...
export class PathwaysDataProvider {
    ...
    this.fmaModes.sub(newFmaModes => {
      switch (newFmaModes?.verticalActive) {
        ...
        case APVerticalModes.CAP:
          this.apAltModeTargetAltitude = newFmaModes.altitideCaptureValue * Avionics.Utils.FEET2METER;
   ...
   */
  private getAllLegVectors(leg: LegDefinition, isActiveLeg: boolean): CircleVector[] {
    const vectors: CircleVector[] = [];
   ...
   */
  private computeBoxesAlongTurn(circleVec: CircleVector, boxIndex: number): number {
   ...
   */
  private computeBoxesAlongGreatArc(vector: CircleVector, boxIndex: number): number {
