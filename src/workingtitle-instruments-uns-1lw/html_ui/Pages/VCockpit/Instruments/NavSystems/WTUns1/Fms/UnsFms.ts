import {
  ...
  FixTypeFlags, FlightPathUtils, FlightPlan, FlightPlanCalculatedEvent, FlightPlanIndicationEvent, FlightPlanLeg, FlightPlanModBatchEvent, FlightPlanner,
  FlightPlannerEvents, FlightPlanOriginDestEvent, FlightPlanSegment, FlightPlanSegmentType, FlightPlanUtils, GeoPoint, GeoPointInterface, GeoPointSubject,
  GNSSEvents, ICAO, IntersectionFacility, LegDefinition, LegDefinitionFlags, LegTurnDirection, LegType, LNavControlEvents, LNavEvents, LNavUtils, MagVar,
  NavEvents, NavMath, NavSourceId, NavSourceType, NearestContext, OneWayRunway, PerformancePlanRepository, RnavTypeFlags, RunwayUtils, SmoothingPathCalculator,
  SpeedRestrictionType, SpeedUnit, SubEvent, Subject, UnitType, UserFacility, VerticalData, VerticalFlightPhase, VerticalFlightPlan, VisualFacility, VNavUtils,
  VorFacility, VorType, Wait
...
export class UnsFms {
   ...
   */
  private hasPrimaryFlightPlan(): boolean {
    return this.flightPlanner.hasFlightPlan(UnsFlightPlans.Active);
  }
  ...
  private buildDepartureLegs(facility: AirportFacility,
    ...
    if (insertProcedureObject.procedureLegs.length === 0 || !ICAO.isFacility(insertProcedureObject.procedureLegs[0].fixIcao, FacilityType.RWY)) {
      if (oneWayRunway) {
    ...
    for (let i = 0; i < finalLegs.length; i++) {
      ...
      if (!isVisual && leg.fixIcao[0] === 'R') {
        ...
        if (resolvedApproach.missedLegs.length > 0) {
          insertProcedureObject.procedureLegs.push(FlightPlanUtils.convertLegRunwayIcaosToSdkFormat(FlightPlan.createLeg(resolvedApproach.missedLegs[0])));
        }
      } else {
        insertProcedureObject.procedureLegs.push(leg);
      }
  ...
  public createDirectTo({ segmentIndex, segmentLegIndex, isNewDto = true, course, facility, turnDirection }: UnsFmsDirectToOptions): void {
    ...
    try {
      ...
      if (segment && leg) {
        ...
        const dtoTargetLeg = this.createDTODirectLeg(leg.leg.fixIcao, leg.leg, course, turnDirection);
   ...
   */
  private createDTODirectLeg(icao: string, leg?: FlightPlanLeg, course?: number, turnDirection?: LegTurnDirection): FlightPlanLeg {
    let legType: LegType;
    if (course === undefined) {
      legType = LegType.DF;
  ...
  public createVerticalDirectTo(
    ...
    const distanceToConstraint = isAutomatic
      ...
      : UnsFmsUtils.getDistanceFromPposToLegEnd(plan, globalIndex);
  ...
  public async insertPVor(pvor: PVorDescription): Promise<boolean> {
    ...
    if (pvor.type === 'outboundRadial') {
      ...
      pvorLegData = { type: LegType.FM, originIcao: fixIcao, course: NavMath.reciprocateHeading(courseToPVorFix) };
    } else {
    ...
    const pvorOriginLeg = FlightPlan.createLeg({ type: LegType.Discontinuity, fixIcao });
