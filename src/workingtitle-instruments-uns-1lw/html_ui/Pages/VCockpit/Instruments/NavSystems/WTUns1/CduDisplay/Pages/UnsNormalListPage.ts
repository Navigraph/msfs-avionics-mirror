import {
  AirportFacility, AirwayObject, DisplayField, Facility, FacilityType, FmcFormatterOutput, FmcPagingEvents, FmcRenderTemplate, GeoPointInterface, ICAO,
...
interface NormalListItem<T extends Facility | AirwayObject = Facility | AirwayObject> {
...
class UnsNormalListPageStore {
  ...
  public selectedAirway = Subject.create<AirwayObject | undefined>(undefined);
  ...
  private airways: NormalListItem<AirwayObject>[] = [];
   ...
   */
  public getItemFromListIndex(index: number): Facility | AirwayObject {
...
export class UnsNormalListPage extends UnsFmcDialogPage<UnsListPageInput, Facility | null> {
  ...
  private ListTitleField = new UnsTextInputField<NormalListCategory, number>(this, {
    ...
    onModified: async (entry: number) => {
      if (this.currentCategory.get() === 'airways') {
        const airway = this.store.getItemFromListIndex(entry - 1) as AirwayObject;
  ...
  private AirwayField = new DisplayField<AirwayObject | undefined>(this, {
