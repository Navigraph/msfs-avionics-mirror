import {
  ActiveLegType, AdcPublisher, AhrsPublisher, AirportFacility, APRadioNavInstrument, Autopilot, AutopilotInstrument, ClockPublisher, EISPublisher, EventBus,
  FacilityLoader, FacilityRepository, FacilityType, FacilityUtils, FlightPathAirplaneSpeedMode, FlightPathCalculator, FlightPlanner, FlightPlannerEvents,
  ...
  LNavComputer, LNavSimVarPublisher, NavComSimVarPublisher, SimVarValueType, SmoothingPathCalculator, Subject, VNode, Wait
...
import { UnsFms, UnsFmsUtils } from './Fms';
...
import { UnsNavDataSimVarPublisher } from './Fms/Navigation/UnsLNavDataEvents';
...
export class WTUns1FsInstrument implements FsInstrument {
  ...
  private readonly lnavPublisher = new LNavSimVarPublisher(this.bus);
  private readonly lnavDataPublisher = new UnsNavDataSimVarPublisher(this.bus);
  ...
  private readonly lnavComputer: LNavComputer | undefined;
  ...
  constructor(public readonly instrument: WTUns1) {
    ...
    if (this.fmsConfig.index === 1) {
      this.lnavComputer = new LNavComputer(
        ...
        this.flightPlanner,
        undefined,
        { maxBankAngle: this.maxBankAngleFn }
      );
    }
    ...
    this.backplane.addPublisher('lnavSimvars', this.lnavPublisher);
    this.backplane.addPublisher('lnavDataSimvars', this.lnavDataPublisher);
  ...
  private async doInit(): Promise<void> {
    ...
    Wait.awaitCondition(() => this.apStateManager?.readyToInitialize ?? true).then(() => this.autopilot?.stateManager.initialize());
