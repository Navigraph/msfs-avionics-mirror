import {
  ActiveLegType, AdcEvents, AdditionalApproachType, AirportFacility, AirportUtils, AirwayObject, AltitudeRestrictionType, ApproachProcedure, ApproachUtils,
  ...
  GeoPointInterface, GNSSEvents, ICAO, IntersectionFacility, LatLonInterface, LegDefinition, LegDefinitionFlags, LegTurnDirection, LegType, MagVar, NavEvents,
  NavMath, NavSourceId, NavSourceType, OneWayRunway, PerformancePlanRepository, RnavTypeFlags, RunwayUtils, SmoothingPathCalculator, SpeedRestrictionType,
  SpeedUnit, Subject, UnitType, UserFacility, UserFacilityUtils, VerticalData, VerticalFlightPhase, VerticalFlightPlan, VisualFacility, VNavDataEvents,
  VNavUtils, VorFacility, VorType
...
export class Epic2Fms {
   ...
   */
  private getPrimaryFlightPlan(): FlightPlan {
  ...
  private async buildApproachLegs(
    ...
    if (lastTransitionLeg && Epic2Fms.isXFLeg(lastTransitionLeg)) {
      for (let i = 0; i < finalLegs.length; i++) {
        ...
        if (isFafOrFacf && Epic2Fms.isXFLeg(finalLeg) && lastTransitionLeg.fixIcao === finalLeg.fixIcao) {
          finalStartIndex = i;
        }
  ...
  public createVerticalDirectTo(
    ...
    const distanceToConstraint = isAutomatic
      ...
      : Epic2FmsUtils.getDistanceFromPposToLegEnd(plan, globalIndex);
  ...
  private planAddOriginDestinationLeg(isOrigin: boolean, segmentIndex: number, airport: AirportFacility, runway?: OneWayRunway): void {
    ...
    if (runway) {
      ...
      leg = FlightPlan.createLeg({
        ...
        altitude1: AirportUtils.getElevation(airport) ?? 0 + UnitType.FOOT.convertTo(50, UnitType.METER)
      });
    }
