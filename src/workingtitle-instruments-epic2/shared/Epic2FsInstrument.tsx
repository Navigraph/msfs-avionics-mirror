import {
  ...
  FlightPathAirplaneSpeedMode, FlightPathCalculator, FlightPlanner, FlightTimerPublisher, FSComponent, FsInstrument, FuelSystemSimVarPublisher,
  GameStateProvider, GNSSPublisher, GPSSatComputer, HEventPublisher, InstrumentBackplane, LNavSimVarPublisher, MappedSubject, MinimumsSimVarPublisher,
...
import { Epic2LNavDataSimVarPublisher } from './Navigation/Epic2LNavDataEvents';
...
export abstract class Epic2FsInstrument implements FsInstrument {
  ...
  protected readonly lnavDataPublisher = new Epic2LNavDataSimVarPublisher(this.bus);
  ...
  protected createSystems(): void {
    ...
    const gpsSystems = this.config.sensors.gpsDefinitions.slice(1, this.config.sensors.gpsCount + 1).map((def, index) => new GpsReceiverSystem(
      ...
      new GPSSatComputer(
        index + 1,
        this.bus,
        'coui://html_ui/Pages/VCockpit/Instruments/NavSystems/Epic2/Assets/Data/gps_ephemeris.json',
        'coui://html_ui/Pages/VCockpit/Instruments/NavSystems/Epic2/Assets/Data/gps_sbas.json',
        5000,
