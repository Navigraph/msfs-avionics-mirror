...
export class NavRadioNavSource<T extends readonly string[]> extends NavSourceBase<T> {
  ...
  private dmeHasDmeSub = this.dmeHasDme.sub((v) => this.setters.get('hasDme')?.(v), false, true);
  ...
  public constructor(bus: EventBus, name: T[number], index: Epic2NavRadioIndex) {
    ...
    this.navComSubscriber.on(`nav_localizer_${index}`).whenChanged().handle(this.setters.get('hasLocalizer')!);
    ...
    this.navComSubscriber.on(`nav_glideslope_${index}`).whenChanged().handle(this.setters.get('hasGlideSlope')!);
    this.navComSubscriber.on(`nav_obs_${index}`).whenChanged().handle(this.setters.get('course')!);
    ...
    this.navComSubscriber.on(`nav_has_nav_${index}`).whenChanged().handle(this.setters.get('hasNav')!);
    this.navComSubscriber.on(`nav_to_from_${index}`).whenChanged().handle(this.setters.get('toFrom')!);
    ...
    dmeEventSubscriber.on(`epic2_nav${index}_dme_hold`).handle(this.setters.get('isDmeHoldActive')!);
