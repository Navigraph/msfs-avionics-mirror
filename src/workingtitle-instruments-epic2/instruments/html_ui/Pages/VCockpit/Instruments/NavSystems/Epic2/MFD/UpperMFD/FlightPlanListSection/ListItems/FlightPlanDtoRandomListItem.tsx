...
export class FlightPlanDtoRandomListItem extends DisplayComponent<FlightPlanDtoRandomListItemProps> {
  ...
  private async getFacilityFromIdent(ident: string): Promise<Facility | null> {
    ...
    const icaos = (await this.props.fms.facLoader.searchByIdent(FacilitySearchType.AllExceptVisual, identUpper)).filter((icao) => ICAO.getIdent(icao) === identUpper);
    if (!icaos || icaos.length == 0) {
      return null;
    } else {
      let resultA = null;
      if (icaos.length == 1 && icaos[0]) {
        resultA = await this.props.fms.facLoader.getFacility(ICAO.getFacilityType(icaos[0]), icaos[0]);
      } else {
