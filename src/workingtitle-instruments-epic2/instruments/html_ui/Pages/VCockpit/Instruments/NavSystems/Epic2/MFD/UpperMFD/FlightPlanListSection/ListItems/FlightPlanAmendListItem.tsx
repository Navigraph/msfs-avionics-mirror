...
export class FlightPlanAmendListItem extends DisplayComponent<FlightPlanAmendListItemProps> {
  ...
  private async getFacilityFromIcao(icaoA: string, icaoB?: string): Promise<[Facility | null, Facility | null]> {
    const waypointsA = (await this.props.fms.facLoader.searchByIdent(FacilitySearchType.AllExceptVisual, icaoA, FlightPlanAmendListItem.MAX_ITEMS)).filter((waypoint) => ICAO.getIdent(waypoint) === icaoA.toUpperCase());
    const waypointsB = icaoB ? (await this.props.fms.facLoader.searchByIdent(FacilitySearchType.AllExceptVisual, icaoB, FlightPlanAmendListItem.MAX_ITEMS)).filter((waypoint) => ICAO.getIdent(waypoint) === icaoB.toUpperCase()) : null;
