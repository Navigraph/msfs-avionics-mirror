...
import {
  APRadioNavInstrument, ArrayUtils, AutothrottleOptions, AutothrottleThrottleIndex, AvionicsSystemState, ClockEvents, ConsumerSubject, EngineType, FSComponent,
  GameStateProvider, GpsSynchronizer, MappedSubject, PluginSystem, SimVarValueType, Subject, Wait, WeightBalanceSimvarPublisher
...
import {
  ...
  DefaultRadioAltimeterDataProvider, DefaultStallWarningDataProvider, Epic2Adsb, Epic2APConfig, Epic2APStateManager, Epic2Autopilot, Epic2DuControlEvents,
  Epic2Fadec, Epic2FlightAreaComputer, Epic2FlightPlans, Epic2Fms, Epic2FsInstrument, Epic2NavDataComputer, Epic2SpeedPredictions, Epic2TcasAuralAlertManager,
  Epic2TcasII, Epic2UserSettingSaveManager, Epic2VSpeedController, FlightPlanListManager, FlightPlanStore, FmcSimVarPublisher, FmsMessageManager,
...
export class Epic2UpperMfdInstrument extends Epic2FsInstrument {
  ...
  constructor(
    ...
    this.fmsMessageManager = new FmsMessageManager(this.bus, this.fms.performancePlanProxy);
  ...
  protected override doInit(): void {
    ...
    this.initPluginSystem().then(() => {
      ...
      this.initFlightPlan().then(() => {
        ...
        this.autopilot.stateManager.initialize(true);
      });
  ...
  private async initFlightPlan(): Promise<void> {
    ...
    // since we're the primary instrument we can create f-plns if needed
    await this.fms.initFlightPlans();
  }
  ...
  private initializeAutopilot(): void {
    ...
    const epic2ApConfig = new Epic2APConfig(this.bus, this.facLoader, this.flightPlanner, this.flightPlanStoreActive,
      this.selectedFmsPosIndex, this.verticalPathCalculator, this.fms.activePerformancePlan, this.pluginSystem);
