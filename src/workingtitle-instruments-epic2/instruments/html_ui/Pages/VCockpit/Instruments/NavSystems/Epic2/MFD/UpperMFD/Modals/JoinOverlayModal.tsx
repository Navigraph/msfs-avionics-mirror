import {
  AirwayObject, ArraySubject, FacilityType, FlightPlanSegmentType, FSComponent, ICAO, IntersectionFacility, LegDefinition, MappedSubject, SetSubject, Subject,
  VNode
...
export class JoinAirwayOverlay extends Modal<JoinAirwayOverlayProps> {
  ...
  private readonly airwayData = ArraySubject.create<AirwayObject>([]);
  ...
  private async getAirways(waypointIcao?: string): Promise<void> {
    ...
    this.airwayData.set([]);
    const airways: AirwayObject[] = [];

    const facType = waypointIcao && waypointIcao.trim().length > 0 && ICAO.getFacilityType(waypointIcao);
    if (facType && (facType === FacilityType.Intersection || facType === FacilityType.VOR || facType === FacilityType.NDB)) {
   ...
   */
  private async getExitWaypoints(airway: AirwayObject | undefined): Promise<void> {
  ...
  private async insertAirway(): Promise<void> {
    ...
    if (entryIcao && exitIcao && airway && displaySegmentIndex !== undefined && displaySegmentLegIndex !== undefined) {
      ...
      const selectedAirway = this.airwayData.getArray().find((airwaySearch) => airway === airwaySearch.name) as AirwayObject;
