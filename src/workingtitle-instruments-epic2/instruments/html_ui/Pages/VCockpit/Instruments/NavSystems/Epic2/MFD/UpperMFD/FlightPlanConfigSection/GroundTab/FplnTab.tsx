import {
  AirportClass, AirportFacility, FacilitySearchType, FacilityType, FSComponent, ICAO, MappedSubject, NearestContext, SetSubject, Subject, Subscription,
  UnitType, VNode, Wait
...
export class FplnTab extends TabContent<FplnTabProps> {
  ...
  private async loadAirportFac(airportIdent: string): Promise<AirportFacility | undefined> {
    const airportIcao = (await this.props.fms.facLoader.searchByIdent(FacilitySearchType.Airport, airportIdent.toUpperCase(), 1))[0];
  ...
  private setNearestOrigin(): void {
    ...
    NearestContext.onInitialized(async (instance) => {
      ...
      for (const airport of instance.airports.getArray()) {
        ...
        if (airport.runways.length === 0 || ICAO.getIdent(airport.icao).length !== 4) {
          continue;
        }
      ...
      if (nearestAirport) {
        this.originValue.set(ICAO.getIdent(nearestAirport.icao).toUpperCase());
      }
    });
