import {
  BasicNavAngleUnit, DefaultUserSettingManager, EventBus, NavAngleUnit, OptionalUserSettingFromManager, Subject, Subscribable, Unit, UnitFamily, UnitType,
  UserSetting, UserSettingConsumerFromManager, UserSettingFromManager, UserSettingManager, UserSettingMap, UserSettingRecord, UserSettingValue
...
/**
 * Setting modes for pressure units.
 */
export enum UnitsPressureSettingMode {
  InHg = 'in hg',
  Millibars = 'millibars',
  hPa = 'hpa',
}
...
export type UnitsUserSettingTypes = {
  ...
  /** The pressure units setting. */
  unitsPressure: UnitsPressureSettingMode;
...
/**
 * Instrument-local versions of display units user settings.
 */
...
export interface UnitsUserSettingManager extends UserSettingManager<UnitsUserSettingTypes> {
  ...
  /** The pressure units to use for the current pressure units setting. */
  readonly pressureUnits: Subscribable<Unit<UnitFamily.Pressure>>;
...
export class DefaultUnitsUserSettingManager implements UnitsUserSettingManager {
  ...
  private readonly _pressureUnits = Subject.create(UnitType.IN_HG);
  public readonly pressureUnits = this._pressureUnits as Subscribable<Unit<UnitFamily.Pressure>>;
  ...
  public constructor(private readonly sourceSettingManager: UserSettingManager<UnitsUserSettingTypes>) {
    ...
    sourceSettingManager.getSetting('unitsPressure').sub(value => {
      switch (value) {
        case UnitsPressureSettingMode.InHg:
          this._pressureUnits.set(UnitType.IN_HG);
          break;
        case UnitsPressureSettingMode.Millibars:
          this._pressureUnits.set(UnitType.MB);
          break;
        case UnitsPressureSettingMode.hPa:
          this._pressureUnits.set(UnitType.HPA);
          break;
        default:
          this._pressureUnits.set(UnitType.IN_HG);
      }
...
export class UnitsUserSettings {
  ...
  private static getDefaultValues(): UnitsUserSettingTypes {
    return {
      ...
      unitsPressure: UnitsPressureSettingMode.InHg,
