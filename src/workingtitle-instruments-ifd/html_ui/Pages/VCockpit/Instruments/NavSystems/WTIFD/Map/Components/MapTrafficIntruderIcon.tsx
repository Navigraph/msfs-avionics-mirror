...
import { MapIfdTrafficModule, MapTrafficMotionVectorMode } from '../Modules/MapIfdTrafficModule';
...
export class MapTrafficIntruderIcon extends AbstractMapTrafficIntruderIcon {
  ...
  private readonly supportTisVector = this.mapIfdTrafficModule.trafficSystem.type === TrafficSystemType.Tis;
   ...
   * @param mapIfdTrafficModule The IFD traffic module for this icon's parent map.
  ...
  constructor(
    ...
    private readonly mapIfdTrafficModule: MapIfdTrafficModule,
  ...
  protected drawIcon(
    ...
    if (this.mapIfdTrafficModule.showIntruderLabel.get()) {
      this.drawIconVSArrow(context, alertLevel);
      this.drawIconAltitudeLabel(context, alertLevel);
    }

    const isAdsbActive = this.mapIfdTrafficModule.adsbOperatingMode.get() !== AdsbOperatingMode.Standby;
  ...
  private drawNoArrowIcon(
    ...
    switch (alertLevel) {
      ...
      case TcasAlertLevel.ProximityAdvisory:
        (alertLevel === TcasAlertLevel.ProximityAdvisory) ? this.drawOutlinedDiamondIcon(context) : this.drawDiamondIcon(context);
        break;
  ...
  /**
   * Draws an outlined (dual border) diamond icon.
   * @param context The canvas rendering context to which to draw the icon.
   */
  private drawOutlinedDiamondIcon(context: CanvasRenderingContext2D): void {
    const size = 0.35 * this.options.iconSize;
    const lineWidthOuter = Math.max(1, 0.18 * this.options.iconSize);
    const lineWidthInner = Math.max(1, 0.10 * this.options.iconSize);
    ...
    context.lineWidth = lineWidthOuter;
    context.stroke();

    context.strokeStyle = Colors.cyan;
    context.lineWidth = lineWidthInner;
    context.stroke();
  }
   ...
   * @param context The canvas rendering context to which to draw the icon.
   */
  private drawDiamondIcon(context: CanvasRenderingContext2D): void {
    const size = 0.35 * this.options.iconSize;
    ...
    context.strokeStyle = Colors.black;
    context.lineWidth = Math.max(1, 0.10 * this.options.iconSize);
  ...
  private drawIconVSArrow(context: CanvasRenderingContext2D, alertLevel: TcasAlertLevel): void {
    ...
    const arrowSize = this.options.iconSize * 1.5;

    context.beginPath();
    context.moveTo(0.67 * arrowSize, -0.16 * arrowSize * vsSign);
    context.lineTo(0.67 * arrowSize, 0.16 * arrowSize * vsSign);
    context.moveTo(0.55 * arrowSize, -0.04 * arrowSize * vsSign);
    context.lineTo(0.67 * arrowSize, -0.18 * arrowSize * vsSign);
    context.lineTo(0.79 * arrowSize, -0.04 * arrowSize * vsSign);

    context.lineWidth = Math.max(1, arrowSize * 0.125);
    context.strokeStyle = 'black';
    context.stroke();

    context.lineWidth = Math.max(1, arrowSize * 0.075);
    switch (alertLevel) {
      case TcasAlertLevel.None:
      case TcasAlertLevel.ProximityAdvisory:
        context.strokeStyle = Colors.cyan;
  ...
  private drawIconAltitudeLabel(context: CanvasRenderingContext2D, alertLevel: TcasAlertLevel): void {
    ...
    if (isAltitudeAbove) {
      ...
      context.strokeText(this.altitudeText, 0, -0.5 * this.options.iconSize);
  ...
  private drawAdsbMotionVector(context: CanvasRenderingContext2D, projection: MapProjection): void {
    const vectorMode = this.mapIfdTrafficModule.motionVectorMode.get();
    ...
    if ((alertLevel === TcasAlertLevel.None || alertLevel === TcasAlertLevel.ProximityAdvisory) || this.options.drawTARAVectorAsNormalVector) {
      ...
      this.drawLookaheadVector(
        ...
        color,
        vector,
        this.mapIfdTrafficModule.motionVectorLookahead.get().asUnit(UnitType.SECOND)
      );
    } else {
