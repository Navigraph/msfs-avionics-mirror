import {
  BitFlags, DebounceTimer, FlightPlanCalculatedEvent, FlightPlanner, GeoPoint, MapOwnAirplanePropsModule, MappedSubject, MapProjection,
  MapProjectionChangeType, MapSystemContext, MapSystemController, MapSystemKeys, ReadonlyFloat64Array, ResourceConsumer, ResourceModerator, Subject, Subscribable,
  Subscription, VecNMath,
...
export interface MapFlightPlanFocusRTRControllerModules {
  ...
  /** Orientation module. */
  [MapKeys.Orientation]?: MapOrientationModule;
}
...
export interface MapFlightPlanFocusRTRControllerControllers {
  /** Range controller. */
  [MapKeys.RangeController]: MapRangeController;
}
...
export interface MapFlightPlanFocusRTRControllerContext {
  ...
  [MapKeys.DesiredOrientationControl]?: ResourceModerator;
...
> {
  ...
  private readonly ownAirplanePropsModule = this.context.model.getModule(MapKeys.OwnAirplaneProps);
  private readonly focusModule = this.context.model.getModule(MapKeys.FlightPlanFocusModule);
  private readonly orientationModule = this.context.model.getModule(MapKeys.Orientation);
  ...
  private readonly desiredOrientationControl = this.context[MapKeys.DesiredOrientationControl];
  ...
  private readonly pendRangeTargetUpdate = (): void => {
    this.isRangeTargetUpdatePending = true;
  };
  ...
  public onAfterMapRender(): void {
    ...
    this.dataProviderSub = this.focusModule.flightPlanner.sub(this.onDataProviderChanged.bind(this), true);
  ...
  private setFlightPlanFocusListenersActive(isActive: boolean): void {
    if (isActive) {
      ...
      this.planCalculatedSub?.resume(true);
    } else {
   ...
   * @param flightPlanner The new flight plan focus data provider.
   */
  private onDataProviderChanged(flightPlanner: FlightPlanner | null): void {
    this.planCalculatedSub?.destroy();

    this.planCalculatedSub = flightPlanner?.onEvent('fplCalculated').handle(this.onFlightPlanCalculated.bind(this), !this.focusModule.isActive.get());
  }

  /**
   * A callback which is called when the flight plan is calculated.
   * @param event The FlightPlanCalculatedEvent.
   */
  private onFlightPlanCalculated(event: FlightPlanCalculatedEvent): void {
    if (event.planIndex !== FlightPlanIndex.ProcedurePreview) {
      return;
    }
  ...
  private updateRangeTargetFromFocus(): void {
    ...
    this.rangeModule.setNominalRangeToFitRadians(targetRange.range);
  }
