...
export interface ScrollListProps extends ComponentProps {
   ...
   * If not provided, then you must provide the totalLengthPx prop.
   */
  ...
  /**
   * The total length of the list, in pixels, along its scroll axis. A sum of the lengths of all list items including the spacing.
   * This is required if the list items can have different heights.
   */
  totalLengthPx?: number | Subscribable<number>;
...
export class ScrollList<P extends ScrollListProps = ScrollListProps> extends DisplayComponent<P> {
  ...
  protected readonly listItemLengthPxProp = SubscribableUtils.toSubscribable(this.props.listItemLengthPx ?? 0, true);
  ...
  protected readonly totalLengthPxProp = SubscribableUtils.toSubscribable(this.props.totalLengthPx, true);

  protected readonly listItemLengthPx = Subject.create(this.listItemLengthPxProp.get() ?? 0);
  ...
  protected readonly _listItemLengthWithMarginPx = MappedSubject.create(
    ([listItemLengthPx, listItemSpacingPx]) => (listItemLengthPx ?? 0) + listItemSpacingPx,
  ...
  protected readonly _totalLengthPx = MappedSubject.create(
    ([listItemLengthPx, listItemSpacingPx, itemCount, totalLengthPx]) => {
      if (totalLengthPx !== undefined) {
        return totalLengthPx;
      }

      const itemLengthsSum = (listItemLengthPx ?? 0) * Math.max(itemCount, 0);
      const itemSpacingsSum = listItemSpacingPx * Math.max(itemCount - 1, 0);

      return itemLengthsSum + itemSpacingsSum;
    },
    this.listItemLengthPx,
    this.listItemSpacingPx,
    this.itemCount,
    this.totalLengthPxProp
  );
    ...
    ? this.itemsPerPage === undefined
      ...
      : MappedSubject.create(
        ([listItemLengthPx, listItemSpacingPx, itemsPerPage]) => {
          return (listItemLengthPx ?? 0) * Math.max(itemsPerPage, 0) + listItemSpacingPx * Math.max(itemsPerPage - 1, 0);
        },
        this.listItemLengthPx,
        this.listItemSpacingPx,
        this.itemsPerPage
  ...
  /**
   * Returns a reference to the element where the static items should be added.
   * @returns A reference to the element where the static items should be added.
   */
  public getStaticContainerRef(): HTMLDivElement {
    return this.translatableRef.instance;
  }
   ...
   * @param animate Whether to animate the scroll.
   */
  public executeScrollTo(pos: number, animate: boolean): void {
