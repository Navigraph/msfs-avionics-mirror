...
/**
 * A request input for {@link DurationField}.
 */
export interface DurationInput extends NumberFieldInput {
  /**
   * The initial duration unit. If not defined, the initial unit will default to a value based on the units mode.
   */
  initialUnit?: Unit<UnitFamily.Duration>;
...
/**
 * A field which allows the user to enter distance in time
 */
export class DurationField {
  private minValue = 0;
  private maxValue = 20;
  public readonly durationValue = Subject.create<string>('');
  ...
  public onKeyPressed(value: string): void {
    ...
    const currentValue = this.durationValue.get();
    ...
    if (!this.inDecimalMode && decimalPart.length === 0) {
      if (wholePart.length < 2) {
        ...
        if (newValue <= this.maxValue) {
          this.durationValue.set(newWhole + '.');
        }
      }
    } else if (this.inDecimalMode && decimalPart.length === 0) {
      this.durationValue.set(wholePart + '.' + value);
    }
  }

  /** @inheritdoc */
  public onRequest(input: DurationInput): void {
    ...
    if (decimalPart === '0') {
      this.durationValue.set(Math.trunc(initialValue) + '.');
    } else {
      this.durationValue.set(valueStr);
    }
    this.touched = false;
    this.inDecimalMode = false;
  }
  ...
  public onBackspacePressed(): void {
    const current = this.durationValue.get();
    ...
    if (decimalPart.length > 0) {
      this.durationValue.set(wholePart + '.');
    } else {
      this.inDecimalMode = false;
      if (wholePart.length > 1) {
        const newWholePart = wholePart.slice(0, -1);
        this.durationValue.set(newWholePart + '.');
      } else {
  ...
  private incrementValue(increment: number): void {
    this.touched = false;
    const currentStr = this.durationValue.get();
    ...
    this.durationValue.set(newValue.toFixed(1));
  }
  ...
  public onEnterPressed(): string {
    const value = this.durationValue.get();
    return value;
  }
  ...
  private getDisplayValue(): string {
    const value = this.durationValue.get() || '0.0';
    if (value.endsWith('.')) {
      return value + '0';
    }
    return value;
    ...
    return (
      <div class='duration-field'>
        <div class="vkb-input vkb-input-flex">
          {this.durationValue.map(() => this.getDisplayValue())}
