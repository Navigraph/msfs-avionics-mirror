...
export class UnitFormatter {
  private static readonly UNIT_TEXT: Partial<Record<string, Partial<Record<string, string>>>> & {
    /** Bearing reference for headings and courses. */
    bearing: Record<UnitsNavAngleSettingMode, string>;
  } = {
    bearing: {
      [UnitsNavAngleSettingMode.Magnetic]: '°',
      [UnitsNavAngleSettingMode.True]: '°T',
    },
    [UnitFamily.Distance]: {
  ...
  /**
   * Gets the label text of a unit.
   * @param unit The unit to return a label for.
   * @returns The unit's label.
   */
  public static unitLabel = <F extends UnitFamily>(unit: Unit<F>): string => UnitFormatter.UNIT_TEXT[unit.family]![unit.name]!;

  /**
   * Gets the unit string for a bearing reference.
   * @param reference The reference type.
   * @returns The bearing label.
   */
  public static bearingLabel = (reference: UnitsNavAngleSettingMode): string => this.UNIT_TEXT.bearing[reference];
}
