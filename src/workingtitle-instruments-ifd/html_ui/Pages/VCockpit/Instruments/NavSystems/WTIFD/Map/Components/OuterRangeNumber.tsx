import {
  ComponentProps, DisplayComponent, EventBus, FSComponent, MappedSubject, Subject, Subscribable, SubscribableUtils, UnitType, VNode
...
/** The properties for the {@link OuterRangeNumber} component. */
interface OuterRangeNumberProps extends ComponentProps {
...
/** The OuterRangeNumber component. */
export class OuterRangeNumber extends DisplayComponent<OuterRangeNumberProps> {
  ...
  private readonly rangeLimit = MappedSubject.create(
    ([range, isActive]: readonly [string, boolean]) => {
      const rangeNM = Number(range);
      const maxRangeCurrentUnit = MapSystemCommon.maxRange[this.unitsSettingManager.distanceUnitsLarge.get().name];
      const maxRangeNM = this.unitsSettingManager.distanceUnitsLarge.get().convertTo(maxRangeCurrentUnit, UnitType.NMILE);

      if (isActive) {
        if (rangeNM >= maxRangeNM) {
          return 'Max';
        }
        if (rangeNM <= MapSystemCommon.minRange) {
          return 'Min';
        }
      } else {
        return '';
    ...
    this.props.mapDataProvider.rangeNumber,
    this.props.mapDataProvider.rangeActive
  );
        ...
        >
          <NumberUnitDisplay
            class={{
              ...
              'hidden': this.rangeLimit.map((v) => !!v),
            }}
            value={this.props.mapDataProvider.rangeNumberWithUnit}
