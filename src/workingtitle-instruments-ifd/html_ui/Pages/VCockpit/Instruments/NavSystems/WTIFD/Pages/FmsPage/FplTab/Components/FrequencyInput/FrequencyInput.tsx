...
import {
  ComponentProps, FSComponent, LifecycleComponent, MappedSubject, MappedSubscribable, MathUtils, MutableSubscribable, RadioType, SetSubject, Subject,
 ...
 */
export class FrequencyInput extends LifecycleComponent<FrequencyInputProps> implements IfdInteractionEventHandler {
  ...
  private readonly comDotShow = Subject.create(false);

  private readonly navComTenMhzSlotRef = FSComponent.createRef<DigitInputSlot>();
  private readonly navComOneMhzSlotRef = FSComponent.createRef<DigitInputSlot>();
   ...
   */
  private readonly inputBaseValue = Subject.create(0);

  public frequency?: MappedSubscribable<number>;
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    this.channelSlotRef.instance.characters.sub(() => {
      const isEmpty = this.channelSlotRef.instance.isEmpty();
      this.comDotShow.set(isEmpty);
  ...
  /**
   * Get the actual current valid user inputted value that is displayed in the field to the user, based on non null slots
   * @returns The combined string of all non-null characters
   */
  public getValue(): string {
    const slots = [
      this.comHundredMhzSlotRef,
      this.navComTenMhzSlotRef,
      this.navComOneMhzSlotRef,
      this.channelSlotRef
    ];

    let result = '';
    for (const slot of slots) {
      const instance = slot?.getOrDefault();
      if (instance) {
        const chars = slot.instance.characters.get().filter(Boolean);
        result += chars.join('');
      }
    }
    return result;
  ...
  public clearValue = (): void => {
    this.comHundredMhzSlotRef.instance.setChar(0, null);
    this.navComOneMhzSlotRef.instance.setChar(0, null);
    this.navComTenMhzSlotRef.instance.setChar(0, null);
  };
    ...
    return (
      <>
        ...
        <DigitInputSlot
          ...
          ref={this.navComOneMhzSlotRef}
        />
        <div class={{ 'hidden': this.comDotShow }}>.</div>
        <ChannelInputSlot
