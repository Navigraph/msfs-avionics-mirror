...
import { EventBus, MappedSubject, NavComEvents, NavComSimVars, NavRadioIndex, NavSourceType, RadioUtils, RunwayUtils, Subject } from '@microsoft/msfs-sdk';
...
export class NavRadioNavSource<T extends readonly string[]> extends NavSourceBase<T> {
  ...
  private readonly runwayNumber = Subject.create<number | null>(null);
  private readonly runwayDesignator = Subject.create<number | null>(null);
  ...
  public constructor(bus: EventBus, name: T[number], index: NavRadioIndex) {
    ...
    navComSubscriber.on(`nav_loc_runway_number_${index}`).whenChanged().handle(this.runwayNumber.set.bind(this.runwayNumber));
    navComSubscriber.on(`nav_loc_runway_designator_${index}`).whenChanged().handle(this.runwayDesignator.set.bind(this.runwayDesignator));
    navComSubscriber.on(`nav_loc_airport_ident_${index}`).whenChanged().handle(this.setters.get('localizerAirportIdent')!.setter);
    ...
    MappedSubject.create(this.runwayNumber, this.runwayDesignator).sub(this.updateRunwayName);
  }
  ...
  private readonly updateRunwayName = (): void => {
    const number = this.runwayNumber.get();
    const designator = this.runwayDesignator.get();
    if (number === null) {
      this.localizerRunway.set(null);
      return;
    }
    this.localizerRunway.set(RunwayUtils.getRunwayNameString(number, designator ?? RunwayDesignator.RUNWAY_DESIGNATOR_NONE));
  };
