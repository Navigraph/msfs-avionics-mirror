...
import { IfdCdiScaleLabel } from '../../Navigation/LNavDataEvents';
...
export class AdsbSensitivityParameters {
  ...
  public selectLevel(
    ...
    const isApproach = cdiScalingLabel === IfdCdiScaleLabel.Approach;

    let level: number;
    if (
      (radarAltFeet === undefined || radarAltFeet > 2350)
      && (!isApproach && cdiScalingLabel !== IfdCdiScaleLabel.Terminal)
    ) {
      if (altFeet > 42000) {
        level = 6;
      ...
      cdiScalingLabel === IfdCdiScaleLabel.Terminal
