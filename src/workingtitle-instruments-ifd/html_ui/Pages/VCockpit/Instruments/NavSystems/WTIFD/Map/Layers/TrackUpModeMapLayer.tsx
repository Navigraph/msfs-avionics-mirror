...
import './TrackUpModeMapLayer.css';

/** The properties for the {@link TrackUpModeMapLayer} component. */
...
export class TrackUpModeMapLayer extends MapLayer<HdgTrkUpModeMapLayerProps> {
  ...
  private readonly targetProjectedOffsetY = MapSystemCommon.hdgTrkUpOffsetY;
    ...
    return (
      <div
        class={{
          ...
          'hidden': this.props.mapDataProvider.mapOrientation.map((v) => v != MapOrientationSettingMode.TrackUp),
        }}
      >
        <MapCompassOffset classname="map-compass-offset-outer" compassSvgSize={this.compassSvgSize} targetProjectedOffsetY={this.targetProjectedOffsetY}>
          ...
          >
            <RoseTicks svgViewBoxSize={this.compassSvgSize}
              ticksRadius={MapSystemCommon.hdgTrkUpCompassRadius + 10}
              tickLength={6}
          ...
          <svg class="range-ring">
            <OutlinedElement
              ...
              stroke-dasharray={(2 * Math.PI * this.rangeRingRadius) / 60}
              d={SVGUtils.describeCircle(this.half, this.half, this.rangeRingRadius)}
            />
          </svg>
        </MapCompassOffset>
        <MapCompassOffset classname="map-compass-offset-inner" compassSvgSize={this.compassSvgSize} targetProjectedOffsetY={this.targetProjectedOffsetY}>
          <HeadingTrackPointer
          ...
          <svg
            ...
            style={{
              position: 'absolute',
              transform: this.props.mapDataProvider.compassUpDirection.map(rot => `rotate3d(0, 0, 1, ${rot * -1}deg)`),
            }}
          >
            <CompassRoseTicks
