import {
  ComponentProps, DisplayComponent, FSComponent, MappedSubject, MutableSubscribable, Subject, Subscribable, SubscribableSet, Subscription,
...
export class CharInput extends DisplayComponent<CharInputProps> {
  ...
  public getIsEditingActive(): Subscribable<boolean> {
  ...
  public moveCursor(direction: 1 | -1, forceSelectionPerSlot: boolean): void {
    ...
    if (direction === 1) {
      ...
      if (firstEmptyIndex !== -1 && cursorPosition >= firstEmptyIndex) {
        return;
      }
    }
    ...
    // If the slot can only take one character, then we skip past it.
    const newCursorPos = this.cursorPosition.get();
    if (this.slots[newCursorPos]?.charArray.length <= 1) {
      this.moveCursor(direction, forceSelectionPerSlot);
    }
  }
