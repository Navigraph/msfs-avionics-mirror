...
import { IfdFlightPlanRouteProvider } from './IfdFlightPlanRouteProvider';

/**
 * A manager for syncing IFD flight plan routes to and from the sim. The manager supports automatically replying to
 * avionics route requests and can be used to manually or automatically load synced flight plan routes.
 */
export class IfdFlightPlanRouteSyncManager {
  private routeManager?: FlightPlanRouteManager;
  private routeLoader?: IfdFlightPlanRouteLoader;
  private routeProvider?: IfdFlightPlanRouteProvider;
  ...
  /**
   * Creates a new instance of IfdFlightPlanRouteSyncManager. The manager is created in an uninitialized state and
  ...
  public init(
    ...
    loader: IfdFlightPlanRouteLoader,
    provider: IfdFlightPlanRouteProvider
  ): void {
    if (!this.isAlive) {
      throw new Error('IfdFlightPlanRouteSyncManager: cannot initialize a dead manager');
    }
  ...
  public startAutoReply(): void {
    if (!this.isAlive) {
      throw new Error('IfdFlightPlanRouteSyncManager: cannot manipulate a dead manager');
    }
   ...
   */
  private onAvionicsRouteRequested(provider: IfdFlightPlanRouteProvider, manager: FlightPlanRouteManager, requestId: number): void {
  ...
  public destroy(): void {
    ...
    this.initPromiseReject('IfdFlightPlanRouteSyncManager: manager was destroyed before initialization');
