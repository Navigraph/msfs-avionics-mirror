import {
  AirportFacilityDataFlags, ApproachUtils, ArrayUtils, BitFlags, FacilityLoader, FacilityType, FlightPlan, FlightPlanLeg, FlightPlanRoute,
  FlightPlanRouteEnrouteLeg, FlightPlanRouteUtils, FlightPlanSegmentType, ICAO, LegDefinitionFlags, LegType, RunwayUtils
} from '@microsoft/msfs-sdk';

/**
 * A utility class for working with IFD flight plan routes.
 */
export class IfdFlightPlanRouteUtils {
  ...
  public static async createRouteFromFlightPlan(
    ...
    if (destinationAirport) {
      ...
      if (
        ...
        // Note: if a visual approach is loaded, then approachIndex is -1, so we don't have to worry about that
        // case here.
    ...
    for (const segment of flightPlan.segmentsOfType(FlightPlanSegmentType.Enroute)) {
      ...
      const enrouteLegs = await Promise.all(
        segment.legs
          ...
          .map(leg => IfdFlightPlanRouteUtils.legToFlightPlanRouteEnrouteLeg(facLoader, leg.leg))
      );
