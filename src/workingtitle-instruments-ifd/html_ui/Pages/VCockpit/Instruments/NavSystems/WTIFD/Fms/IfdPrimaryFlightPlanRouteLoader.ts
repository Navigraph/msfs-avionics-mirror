import {
  AirportFacility, AirportUtils, AirwayData, AirwayType, ApproachIdentifier, ArrayUtils, Facility, FacilityType, FacilityUtils, FlightPlan, FlightPlanUtils,
  GeoMath, GeoPoint, ICAO, IcaoType, IcaoValue, IntersectionFacility, LatLonInterface, LegDefinition, LegType, OneWayRunway, Procedure, ReadonlyFlightPlanRoute,
  RunwayIdentifier, RunwayUtils, SimVarValueType, UnitType, UserFacility, UserFacilityUtils
} from '@microsoft/msfs-sdk';

import { Fms } from './Fms';
import { FmsUtils } from './FmsUtils';
import { IfdFlightPlanRouteLoader } from './IfdFlightPlanRouteLoader';
import { IfdFlightPlanRouteUtils } from './IfdFlightPlanRouteUtils';

/**
 * Configuration options for {@link IfdPrimaryFlightPlanRouteLoader}.
 */
export type IfdPrimaryFlightPlanRouteLoaderOptions = {
  /**
   ...
   * {@link IfdFlightPlanRouteUtils.DEFAULT_LOAD_ROUTE_BATCH_NAME}.
   */
 ...
 */
export class IfdPrimaryFlightPlanRouteLoader implements IfdFlightPlanRouteLoader {
  ...
  /**
   * Creates a new instance of IfdPrimaryFlightPlanRouteLoader.
   ...
   */
  public constructor(private readonly fms: Fms, options?: Readonly<IfdPrimaryFlightPlanRouteLoaderOptions>) {
    this.loadRouteBatchName = options?.loadRouteBatchName ?? IfdFlightPlanRouteUtils.DEFAULT_LOAD_ROUTE_BATCH_NAME;
    ...
    if (this.userFacilityScope.length > 4) {
      throw new Error(`IfdPrimaryFlightPlanRouteLoader: invalid user facility scope "${this.userFacilityScope}" (exceeds maximum length of 4 characters)`);
    }

    if (this.userFacilityIdentPrefix.length > 3) {
      throw new Error(`IfdPrimaryFlightPlanRouteLoader: invalid user facility ident prefix "${this.userFacilityIdentPrefix}" (exceeds maximum length of 3 characters)`);
    }
  }
  ...
  private async loadFromRoute(opId: number, route: ReadonlyFlightPlanRoute): Promise<boolean> {
    ...
    try {
      ...
      if (ICAO.isValueFacility(route.departureAirport, FacilityType.Airport)) {
        ...
        if (origin) {
          ...
          if (departureIndexes.procedureIndex >= 0) {
            const result = await this.fms.loadDeparture(
              origin.airport,
            ...
            if (!result || opId !== this.loadOpId) {
              return false;
            }
          } else {
            await this.fms.setOrigin(origin.airport, origin.runway);
          }
        }
      }
      ...
      if (ICAO.isValueFacility(route.destinationAirport, FacilityType.Airport)) {
        ...
        if (destination) {
          await this.fms.setDestination(destination.airport, destination.runway);
          ...
          if (arrivalIndexes.procedureIndex >= 0) {
            const result = await this.fms.loadArrival(
              destination.airport,
          ...
          if (
            ...
            const result = await this.fms.insertApproach(
              destination.airport,
      ...
      if (leg.hasLatLon || leg.hasPointBearingDistance) {
        if (currentUserFacilityIndex < 999) {
          ...
          if (latLon) {
            ...
            const inserted = this.fms.insertWaypoint(userFacility, currentSegment.segmentIndex);
          ...
          console.warn('IfdPrimaryFlightPlanRouteLoader: cannot load lat/lon or PBD enroute leg due to user waypoint limit reached');
        }
      } else {
        if (ICAO.isValueFacility(leg.fixIcao)) {
            ...
            if (leg.via !== '') {
              ...
              if (airwayEntryFacility) {
                ...
                if (airwayExitFacility) {
                  ...
                  console.warn(`IfdPrimaryFlightPlanRouteLoader: could not load airway '${leg.via}' to fix ${leg.fixIcao.ident} - the fix is not a valid exit waypoint`);
                }
              } else {
                console.warn(`IfdPrimaryFlightPlanRouteLoader: could not load airway '${leg.via}' to fix ${leg.fixIcao.ident} - no valid entry waypoint`);
              }
            }
              ...
              if (this.flattenAirways) {
                ...
                if (sign !== 0) {
                  ...
                  for (let i = 1; i < count; i++) {
                    ...
                    if (facilityToInsert) {
                      const insertedLeg = this.fms.insertWaypoint(facilityToInsert, currentSegment.segmentIndex);
                      firstAirwayLeg ??= insertedLeg;
                    }
                  }

                  // Insert exit waypoint.
                  const insertedLeg = this.fms.insertWaypoint(facility, currentSegment.segmentIndex);
                  firstAirwayLeg ??= insertedLeg;
          ...
          console.warn(`IfdPrimaryFlightPlanRouteLoader: enroute fix with ICAO '${ICAO.valueToStringV1(leg.fixIcao)}' has invalid facility type`);
        }
      }
  ...
  private async retrieveFacility(icao: IcaoValue): Promise<Facility | null> {
    try {
      ...
      console.warn(`IfdPrimaryFlightPlanRouteLoader: unable to retrieve facility with ICAO '${ICAO.valueToStringV1(icao)}'`);
    }
    ...
    if (ICAO.isValueFacility(airportIcao, FacilityType.Airport)) {
      ...
      if (airport) {
        ...
        if (runwayIdentifier.number !== '') {
          ...
          if (!runway) {
            console.warn(`IfdPrimaryFlightPlanRouteLoader: unable to retrieve find runway with designation '${designation}' in airport ${airportIcao.ident}`);
          }
        }
  ...
  private retrieveDepartureArrivalIndexes(
    ...
    if (procedureName !== '' && result.procedureIndex < 0) {
      console.warn(`IfdPrimaryFlightPlanRouteLoader: unable to find procedure with name '${procedureName}' in airport ${ICAO.getAirportIdentFromStringV1(airport.icao)}`);
    }

    if (result.procedureIndex >= 0 && transitionName !== '' && result.enrouteTransitionIndex < 0) {
      console.warn(`IfdPrimaryFlightPlanRouteLoader: unable to find enroute transition with name '${transitionName}' in procedure ${ICAO.getAirportIdentFromStringV1(airport.icao)}.${procArray[result.procedureIndex].name}`);
    }
      ...
      switch (ICAO.getFacilityTypeFromValue(icao)) {
        ...
        case FacilityType.Intersection:
          try {
            ...
            console.warn(`IfdPrimaryFlightPlanRouteLoader: unable to retrieve waypoint with ICAO '${ICAO.valueToStringV1(icao)}'`);
          }
      }
    }
  ...
  > {
    if (ICAO.valueEquals(entryFacility.icaoStruct, exitFacility.icaoStruct)) {
      console.warn(`IfdPrimaryFlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the fix is the same as the entry fix`);
      return null;
    }
    ...
    try {
      ...
      if ((exitIndex = airway.waypoints.findIndex(airwayFix => ICAO.valueEquals(airwayFix.icaoStruct, exitFacility.icaoStruct))) >= 0) {
        ...
        if ((entryIndex = airway.waypoints.findIndex(airwayFix => ICAO.valueEquals(airwayFix.icaoStruct, entryFacility.icaoStruct))) >= 0) {
          ...
          console.warn(`IfdPrimaryFlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the entry fix ${ICAO.tryValueToStringV2(entryFacility.icaoStruct)} is not part of the specified airway`);
        }
      } else {
        // Should never happen.
        console.warn(`IfdPrimaryFlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the fix is not part of the specified airway`);
      }
    } catch {
      console.warn(`IfdPrimaryFlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the fix is not part of the specified airway`);
    }
  ...
  private getInUseUserFacilityIndexesFromPlan(plan: FlightPlan): number[] {
    ...
    for (const leg of plan.legs()) {
      // IFD flight plans can only reference user facilities as terminator fixes, so we only need to check fixIcao.
