import {
  ComponentProps, DisplayComponent, FSComponent, MathUtils, MutableSubscribable, SetSubject, Subscribable, SubscribableSet, SubscribableUtils, Subscription,
  VNode
...
export class DigitInputSlot extends DisplayComponent<DigitInputSlotProps> {
  ...
  /** The chracters entered in the slot.
   * @returns Subscribable - character array
   */
  public get characters(): Subscribable<readonly (string | null)[]> {
    return this.slotRef.instance.characters;
  }
  ...
  public isEmpty(): boolean {
    return this.slotRef.instance.charsEmpty();
  }
  ...
  private canSetChar(index: number, character: string | null, force: boolean): boolean {
    ...
    // for slots with multiple chars we need to consider them all for the min/max
    const characters = this.characters.get();
    let maxPossibleValue = 0;
    let minPossibleValue = 0;

    // for chars before this one we consider the actual value
    for (let i = 0; i < index; i++) {
      const slotValue = Number(characters[i]);
      if (!isNaN(slotValue)) {
        const place = Math.max(0, this.characterCount - i - 1);
        maxPossibleValue += slotValue * Math.pow(10, place);
        minPossibleValue += slotValue * Math.pow(10, place);
      }
    }

    // for chars afer we consider the highest value for the max possible, and lowest (0) for min possible
    for (let i = index + 1; i < this.characterCount; i++) {
      const slotValue = Number(characters[i]);
      if (!isNaN(slotValue)) {
        const place = Math.max(0, this.characterCount - i - 1);
        maxPossibleValue += slotValue * Math.pow(10, place);
    ...
    maxPossibleValue += parsed * Math.pow(10, place);

    return maxPossibleValue >= this.minValue.get() && (this.wrap.get() ? minPossibleValue < this.maxValue.get() : minPossibleValue <= this.maxValue.get());
  }
