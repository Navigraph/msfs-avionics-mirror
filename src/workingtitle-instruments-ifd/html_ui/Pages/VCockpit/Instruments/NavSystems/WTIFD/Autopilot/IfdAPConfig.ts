import {
  ...
  APVerticalModes, APVSDirector, EventBus, NavMath, PlaneDirector, UnitType, VNavManager
} from '@microsoft/msfs-sdk';

import { IfdAutopilotOptions } from '../IfdOptions';
import { IFdVNavGlidepathGuidance } from '../Navigation/Vnav/IfdVnavTypes';
import { IfdAPUtils } from './IfdAPUtils';

/**
 * An IFD Autopilot Configuration.
 */
export class IfdAPConfig implements APConfig {
  public defaultLateralMode = APLateralModes.ROLL;
  public defaultVerticalMode = APVerticalModes.PITCH;

  public supportFlightDirector = this.options.independentFlightDirector;

  public readonly apElectricalLogic = this.options.electricalLogic;
   ...
   * @param glidepathGuidance The approach glidepath guidance to send to the autopilot's GP director.
   * @param cdiId The CDI ID to use.
   * @param options The AP options to use.
   */
  constructor(
    private readonly bus: EventBus,
    private readonly gpsSteerCommand: Accessible<Readonly<APGpsSteerDirectorSteerCommand>>,
    private readonly glidepathGuidance: Accessible<Readonly<IFdVNavGlidepathGuidance>>,
    public readonly cdiId: string,
    private readonly options: IfdAutopilotOptions,
  ) {
  ...
  protected createGpssDirector(apValues: APValues): APGpsSteerDirector {
    return new APGpsSteerDirector(
      ...
      {
        maxBankAngle: this.options.maxBankAngle,
        canActivate: this.options.enableNavArming
          ? IfdAPUtils.gpssCanActivate
          : () => true
      }
    );
  }
  ...
  protected createBcDirector(apValues: APValues): APBackCourseDirector | undefined {
    if (!this.options.enableBackCourse) {
      return undefined;
    } else {
  ...
  protected createGpDirector(apValues: APValues): PlaneDirector | undefined {
    return new APGPDirector(this.bus, apValues, {
      ...
      canArm: IfdAPUtils.glidepathCanArm.bind(undefined, apValues),
