import {
  ArrayUtils, DebounceTimer, DisplayComponent, FSComponent, MappedSubject, MappedSubscribable, Subject, Subscribable, SubscribableArray,
...
export class DynamicList<DataType extends DynamicListData> {
  ...
  private _variableItemLengthMapped?: MappedSubscribable<number>;
  private readonly _variableItemLength = Subject.create(0);
  private readonly _fixedItemLength = Subject.create(0);

  private readonly _totalListLength = MappedSubject.create(([fixedLength, variableLength]) => fixedLength + variableLength, this._variableItemLength, this._fixedItemLength);
  /** The total length of the list, including all items. A sum of each item's height and their spacing. */
  public readonly totalListLength = this._totalListLength as Subscribable<number>;

  private readonly lengthCalculatorDebounce = new DebounceTimer();
  ...
  private readonly heightSubjects = new Map<DataType, Subscribable<number> | MappedSubscribable<number>>();
   ...
   * @param spacingPx The amount of space, in pixels, between each list item, for calculating total length.
   * @param keepSpaceBeforeFirstItem Whether to keep space before the first item in the list.
   * @param keepSpaceAfterLastItem Whether to keep space after the last item in the list.
   ...
   */
  public constructor(
    public readonly data: SubscribableArray<DataType>,
    ...
    private readonly keepSpaceBeforeFirstItem = false,
    private readonly keepSpaceAfterLastItem = false,
   ...
   */
  public getRenderedItem(index: number): IfdListItemWrapper | undefined {
    return this.renderedNodes[index]?.instance as IfdListItemWrapper;
  }
  ...
  private onDataChanged(index: number, type: SubscribableArrayEventType, data: DataType | readonly DataType[] | undefined): void {
    ...
    this.calculateFixedLengthDebounced();
  }
  ...
  private addDataItem(dataItem: DataType, indexToAdd: number): void {
    ...
    // Handle item lengths for variable items
    if (SubscribableUtils.isSubscribable(dataItem.heightPx)) {
      const subject = dataItem.isVisible === undefined ?
        dataItem.heightPx : MappedSubject.create(([height, visibility]) => visibility ? height : 0, dataItem.heightPx, dataItem.isVisible);
      this.heightSubjects.set(dataItem, subject);

      this.rebuildVariableItemLength();
    }
  ...
  /**
   * Calculates the total length of the list, debounced.
   */
  private calculateFixedLengthDebounced(): void {
    this.lengthCalculatorDebounce.schedule(this.calculateFixedLength.bind(this), 0);
  }

  /**
   * Calculates the total length of the list by summing the heights of all visible items.
   */
  private calculateFixedLength(): void {
    let fixedLength = -this.spacingPx;

    for (const item of this.data.getArray()) {
      if (!SubscribableUtils.isSubscribable(item.heightPx)) {
        fixedLength += (item.isVisible === undefined || item.isVisible.get() === true) ? item.heightPx + this.spacingPx : 0;
      }
    }

    if (this.keepSpaceBeforeFirstItem) {
      fixedLength += this.spacingPx;
    }
    if (this.keepSpaceAfterLastItem) {
      fixedLength += this.spacingPx;
    }

    this._fixedItemLength.set(Math.max(fixedLength, 0));
  }

  /**
   * Rebuilds the variable item length mapped subject from current height subjects.
   */
  private rebuildVariableItemLength(): void {
    this._variableItemLengthMapped?.destroy();
    this._variableItemLengthMapped = MappedSubject.create(
      ([...heights]) => heights.reduce((a, b) => {
        const varSpacing = (b === 0 ? 0 : this.spacingPx);
        return a + b + varSpacing;
      }, 0),
      ...Array.from(this.heightSubjects.values())
    );
    this._variableItemLengthMapped.pipe(this._variableItemLength);
  }
  ...
  private removeDataItem(data: DataType, index: number): void {
    ...
    const heightSubject = this.heightSubjects.get(data);
    if (heightSubject) {
      (heightSubject as any).destroy?.(); // If it's a mapped subject then we also need to destroy it
      this.heightSubjects.delete(data);
  ...
  private onDataCleared(): void {
    ...
    this.heightSubjects.forEach(x => {
      (x as any)?.destroy?.();
    });
    this.heightSubjects.clear();
    this._variableItemLengthMapped?.destroy();
    this._variableItemLength.set(0);
