import {
  ...
  DebounceTimer, DisplayComponent, FSComponent, Subject, Subscribable, SubscribableUtils, Subscription, VNode
...
export class AfcsStatusBox extends DisplayComponent<AfcsStatusBoxProps> {
  ...
  public onAfterRender(): void {
    ...
    this.fmaDataSub = this.fmaData.sub(data => {
      this.updateFromFmaData(data, this.lastData);
  ...
  private onFdActiveChanged(isActive: boolean): void {
    if (isActive) {
      ...
      this.updateFromFmaData(fmaData);
   ...
   */
  private updateFromFmaData(data: FmaData, lastData?: FmaData): void {
  ...
  private getVerticalArmedStrings(
    ...
    if (armedMode === APVerticalModes.PATH) {
      armedString = 'VNAV';
