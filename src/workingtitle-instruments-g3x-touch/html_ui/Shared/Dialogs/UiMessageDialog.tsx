...
import { TouchBackground } from '../Components/TouchBackground/TouchBackground';
import { UiTouchButton } from '../Components/TouchButton/UiTouchButton';
import { AbstractUiView } from '../UiSystem/AbstractUiView';
import { UiDialogResult, UiDialogView } from '../UiSystem/UiDialogView';
...
export type UiMessageDialogInput = {
  ...
  /** Whether the dialog should close itself when the user touches its background. Defaults to false. */
...
export class UiMessageDialog extends AbstractUiView implements UiDialogView<UiMessageDialogInput, boolean> {
  ...
  private readonly backgroundRef = FSComponent.createRef<TouchBackground>();
  private readonly isBackgroundVisible = Subject.create(false);
  ...
  public request(input: UiMessageDialogInput): Promise<UiDialogResult<boolean>> {
    ...
    return new Promise<UiDialogResult<boolean>>((resolve) => {
      ...
      this.isBackgroundVisible.set(closeOnBackgroundTouch);
  ...
  private closeSelf(): void {
    this.props.uiService.closePfdPopup(popup => popup.ref === this) || this.props.uiService.closeMfdPopup(popup => popup.ref === this);
  }

  /**
   * Responds to when this dialog's background is pressed.
   */
  private onBackgroundPressed(): void {
    this.closeSelf();
  }
  ...
  /**
   * Responds to when this dialog's accept button is pressed.
   */
    ...
    return (
      <>
        <TouchBackground
          ...
          isVisible={this.isBackgroundVisible}
          onPressed={this.onBackgroundPressed.bind(this)}
          class='ui-message-dialog-touch-background'
        />
  ...
  public destroy(): void {
    ...
    this.backgroundRef.getOrDefault()?.destroy();
