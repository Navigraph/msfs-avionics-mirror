...
import { WTLineFlightPlanRouteLoader } from './WTLineFlightPlanRouteLoader';
...
export class WTLineFlightPlanRouteSyncManager<T extends FlightPlanIndexTypes<number, number>> {
  ...
  private routeLoader?: WTLineFlightPlanRouteLoader<T>;
   ...
   */
  public constructor(private readonly fms: WTLineFms<T>) {
    ...
    do {
      ...
      try {
        if (this.fms.hasPrimaryFlightPlan()) {
          route = await WTLineFlightPlanRouteUtils.createRouteFromFlightPlan(
            this.fms.facLoader,
