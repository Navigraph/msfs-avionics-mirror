import {
  ...
  FacilityUtils, FlightPlan, FlightPlanSegmentType, FlightPlanUtils, ICAO, IcaoType, IcaoValue, IntersectionFacility,
...
import { WTLineFms } from './WTLineFms';
import { WTLineFlightPlanRouteUtils } from './WTLineFlightPlanRouteUtils';
import { WTLinePilotWaypointUtils } from '../navigation/WTLinePilotWaypointUtils';
...
export abstract class WTLineFlightPlanRouteLoader<T extends FlightPlanIndexTypes<U, V>, U extends number = FlightPlanIndexType<T>, V extends U = MainFlightPlanIndexType<T>> {
   ...
   */
  public constructor(protected readonly fms: WTLineFms<T>, private readonly mainFlightPlanIndex: T['main']) {
  }

  /**
   * Sets the cruising altitude for a particular flight plan
   * @param mainPlanIndex the main flight plan index to set the cruising altitude for
   * @param cruiseAlt the cruising altitude value, in feet
   */
  public abstract setCruiseAltitude(mainPlanIndex: V, cruiseAlt: number): void;
  ...
  private async loadFromRoute(opId: number, route: ReadonlyFlightPlanRoute, exec: boolean): Promise<boolean> {
    ...
    try {
      ...
      plan = this.fms.getPlanToEdit(this.mainFlightPlanIndex);
      ...
      batchUuid = plan.openBatch(WTLineFlightPlanRouteUtils.DEFAULT_LOAD_ROUTE_BATCH_NAME);

      this.fms.emptyFlightPlanForEdit(this.mainFlightPlanIndex, true);
  ...
  private async loadEnrouteLegs(opId: number, plan: FlightPlan, route: ReadonlyFlightPlanRoute): Promise<LegDefinition | null> {
    ...
    for (const leg of route.enroute) {
      ...
      if (leg.hasLatLon || leg.hasPointBearingDistance) {
        if (!WTLinePilotWaypointUtils.isLimitReached(userFacilities)) {
          let facility: UserFacility | undefined;

          if (leg.hasLatLon) {
            const ident = WTLinePilotWaypointUtils.nextAutoGeneratedName(userFacilities, 'LL');
            const icao = ICAO.value(IcaoType.User, '', WTLineFacilityUtils.USER_FACILITY_SCOPE, ident);
            ...
            if (referenceFacility) {
              const ident = WTLinePilotWaypointUtils.nextAutoGeneratedName(userFacilities, leg.referenceIcao.ident);
              const icao = ICAO.value(IcaoType.User, '', WTLineFacilityUtils.USER_FACILITY_SCOPE, ident);
        ...
        if (ICAO.isValueFacility(leg.fixIcao)) {
          ...
          if (facility) {
            ...
            if (airway) {
              ...
              // Inserting a waypoint might create an enroute segment right after the current one. If that is the
              // case, use that now.
              const nextSegment = plan.tryGetSegment(currentSegment.segmentIndex + 1);
